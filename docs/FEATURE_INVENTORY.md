...
|---|---|---|---|---|---|---|---|---|---|
| C01 | Trip logging | Create, edit, save, and delete fishing trips. | Implemented | New Trip, trip row Edit, summary Edit; trusted operator | Trip plus nested people, gear, catches, lost fish, media | `templates/partials/dialogs/trip-editor.html`, `trip-editor.js`, `trip-rows.js`, `trip-save.js`, `app.js` | `trips[]` | GET/PUT `/api/logbook` | Trips, Trip dialog |
...
| C11b | Radius-based fishing spots | Create global named radius circles, automatically match landed catch GPS, and allow sticky manual or explicit no-spot choices. | Implemented | Settings > Waterbodies and landed-catch rows | Leaflet/OpenStreetMap; nearest containing radius; manual selection wins | `settings.js`, `app-state.js`, `trip-editor.js` | `spots[]`, catch `spotId`/`spotAssignmentMode` | GET/PUT `/api/logbook` | Settings, Trip dialog, Summary, Map |
| C11a | Expedition tracking | Create multi-day fishing vacations, assign trips, and review aggregate totals and member trips. | Implemented | Expeditions page and Trip Basics selector | One optional expedition per trip; explicit vacation dates | `expeditions.js`, `trip-editor.js`, `logbook_store.py` | `expeditions[]`, `trip.expeditionId` | GET/PUT `/api/logbook` | Expeditions, Trip dialog |
...
| C15 | Deepest-rigger marker | Mark an individual main-downrigger catch as the deepest rigger for analysis. | Implemented | Downrigger catch row | Trolling presentation and non-cheater target | `form-utils.js`, `stats.js` | `catch.deepestRigger` | GET/PUT `/api/logbook` | Trip dialog, Stats |
...
| C22 | Catch GPS override | Accept manual latitude/longitude and prefer it over media GPS. | Implemented | Catch row coordinates | Coordinate validation | `photos.js`, `maps.js` | `manualCoordinates`, `coordinates` | GET/PUT `/api/logbook` | Trip dialog, Maps |
...
| C25 | Lure/bait library | CRUD reusable lures with type, brand/model, color, notes, media, usage, and outcomes. Natural/live bait has no dedicated model. | Partial | Gear > Baits; inline New Lure | Upload service, trip references | `gear-dialogs.js`, `gear-inventory.js`, `templates/partials/dialogs/gear.html` | `lures[]`, `lureId` | Logbook and upload APIs | Gear, Trip dialog, Stats |
| C26 | Flasher library | CRUD trolling flashers/attractors with metadata, media, usage, and outcomes. | Implemented | Gear > Flashers; inline New Flasher | Upload service | `gear-dialogs.js`, `gear-inventory.js` | `flashers[]`, `flasherId` | Logbook and upload APIs | Gear, Trip dialog, Stats |
| C27 | Reel inventory | CRUD reels with specifications, purchase data, notes, media, and fish counts. | Implemented | Gear > Reels | Upload service, combos, setup references | `gear-dialogs.js`, `gear-inventory.js` | `reels[]`, `reelId` | Logbook and upload APIs | Gear, Trip dialog |
| C28 | Rod inventory | CRUD rods with specifications, purchase data, notes, media, and fish counts. | Implemented | Gear > Rods | Upload service, combos, setup references | `gear-dialogs.js`, `gear-inventory.js` | `rods[]`, `rodId` | Logbook and upload APIs | Gear, Trip dialog |
...
| E03 | Gear media uploads | Attach image/video records to lures, flashers, rods, and reels. | Implemented | Gear dialogs | Local filesystem | `gear-core.js`, `gear-dialogs.js`, `media_service.py` | Gear image fields + files | POST `/api/uploads/{category}` | Gear, Gallery |
...
| E09 | Global fish/media map | Map catch GPS and geotagged trip media with species/category filtering and list links. | Implemented | Map route | Leaflet + OpenStreetMap tiles | `maps.js` | Catch/media coordinates | File GET routes | Map |
| E10 | Trip map | Map one trip’s catch and media records with filtering. | Implemented | Trip Summary | Leaflet + tile service | `maps.js` | Nested trip coordinates | None | Trip Summary |
...
| U05 | Direct-file local fallback | Opening `index.html` redirects to generated `standalone.html`, which loads/saves localStorage when Flask is unavailable. Uploads/weather do not work in this mode. | Implemented / Partial | Open local HTML file | Browser localStorage | `index.html`, `standalone.html`, `app-state.js` | None | Same SPA UI |
